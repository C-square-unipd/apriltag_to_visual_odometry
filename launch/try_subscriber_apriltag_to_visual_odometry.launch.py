...
def generate_launch_description():
    ...
    # params_file = os.path.join(share_dir, 'config', 'apriltag_virtual_map.yaml')
    params_file = os.path.join(share_dir, 'config', 'apriltag_virtual_map_3450x3450.yaml')
    with open(params_file, 'r') as f:
        params = yaml.safe_load(f)['apriltag_to_visual_odometry']['ros__parameters']
    return LaunchDescription([
        launch_ros.actions.Node(
            package='apriltag_to_visual_odometry',
            executable='try_subscriber_apriltag_to_visual_odometry',
            output='screen',
            parameters=[params]
        ),
    ])
